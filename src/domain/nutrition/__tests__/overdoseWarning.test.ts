...
describe('computeOverdoseWarnings', () => {
  it('fires a warning when current exceeds the tolerable upper limit', () => {
    ...
    const warnings = computeOverdoseWarnings(micros, 'vi');
  ...
  it('stays empty when nothing exceeds its upper limit', () => {
    ...
    expect(computeOverdoseWarnings(micros, 'vi')).toHaveLength(0);
  });
  ...
  it('produces a gentle, non-alarmist referential message with no medical/diagnostic wording', () => {
    ...
    const [warning] = computeOverdoseWarnings(micros, 'vi');
    expect(warning.message).toContain('tham khảo');
    expect(warning.message).toContain('Canxi');
    expect(warning.message.toLowerCase()).not.toContain('nguy hiểm');
    expect(warning.message.toLowerCase()).not.toContain('chẩn đoán');
  });
