...
| **S-T3** | 🆕 Undo nút Vận động (movement event) in-app | ⏸ TRÌ HOÃN có lý do — thiết kế cần xác nhận (2026-07-15): movement event không lưu trong `intakeLog`, `IntakeEvent` không lưu `stepType` để reverse goal growth chính xác. Bây giờ quick-tap nút thêm Vận động → xoá không được (không có dữ liệu để undo). Cần quyết định: (A) lưu movement vào intakeLog + thêm stepType field, (B) tạo ledger riêng cho movement. Xem `.ai/parallel-reports/S-T-movement-energy-sync.md` mục "Known limitation" | logic-backend | — | design decision |
